...
timeperiod_data <- '2021_Q2'

# what's the name of the file to be converted?
indata_file <- 'data/ki/original_data/APC data till KB Q2 2021.xlsx'
...
converter <- converter %>%
    ...
    mutate(euro = format(round(0.0986*sek, 2), nsmall = 2)) %>% #valutakurs 2021 q2 hämtad från https://www.riksbank.se/sv/statistik/sok-rantor--valutakurser/ (månadsgenomsnitt)
    select(-sek) %>%
